...
private:
  static std::size_t constexpr BufWidth = sizeof(std::size_t) * 8;
...
public:
  ...
  void putBits(std::size_t const code, std::size_t const bits) {
    if (bits_left >= bits)
      ...
      for (std::int32_t k = static_cast<std::int32_t>(BufWidth) - 8; k >= 0; putChar((buffer >> k) & 0xff), k -= 8);
  ...
  void flush() {
    if (bits_left !=BufWidth) {
      buffer <<= bits_left;
      for (std::int32_t i = 8; i <= static_cast<std::int32_t>(BufWidth - bits_left); putChar((buffer >> (BufWidth - i)) & 0xff), i += 8);
    }
  }
